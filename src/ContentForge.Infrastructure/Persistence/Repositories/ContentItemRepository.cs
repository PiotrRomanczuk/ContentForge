...
{
    public ContentItemRepository(ContentForgeDbContext context, ILoggerFactory? loggerFactory = null)
    ...
    {
        Logger.LogDebug("GetByStatus({Status})", status);
        return await DbSet
    ...
    {
        Logger.LogDebug("GetByBotName({BotName})", botName);
        return await DbSet
    ...
    {
        Logger.LogDebug("GetPendingApproval(skip: {Skip}, take: {Take})", skip, take);
        return await DbSet
    ...
    {
        Logger.LogDebug("GetScheduledForPublishing(until: {Until})", until);
        return await DbSet
    ...
    public async Task<int> GetCountByStatusAsync(
        ContentStatus status, CancellationToken cancellationToken = default)
    {
        Logger.LogDebug("GetCountByStatus({Status})", status);
        return await DbSet.CountAsync(c => c.Status == status, cancellationToken);
    }
        ...
        IEnumerable<Guid> ids, ContentStatus status, CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation("BulkUpdateStatus: {Count} items → {Status}", idList.Count, status);
        await DbSet
